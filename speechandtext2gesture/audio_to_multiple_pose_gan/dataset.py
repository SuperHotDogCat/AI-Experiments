...
def threadsafe_generator(f):
    ...
    def g(*a, **kw):
        return threadsafe_iter(f(*a, **kw)) #iterを追加
...
def Speech2GestureData(cfg: dict, df: pd.DataFrame, batch_size: int = 32)->np.ndarray:
    """
    データを取り出す関数
    """
    process_row, decode_pose = get_processor(cfg) #{"processor": "audio_to_pose", "input_shape": [None, AUDIO_SHAPE]}など
    X, Y = generate_batch(df, process_row, batch_size)
    return X,Y
