...
from audio_to_multiple_pose_gan.torch_layers import to_motion_delta_2, keypoints_to_train, keypoints_regloss
...
def train(G_model, D_model, optimizer_g, optimizer_d, df, process_row,batch_size,device):
    ...
    fake_pose_Y_input = torch.concatenate([fake_pose_Y[:,:-1,:],to_motion_delta_2(fake_pose_Y[:,:-1,:])], dim =1)
    D_loss = 0
