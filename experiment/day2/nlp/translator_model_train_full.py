...
model = TranslatorModel(dataset.english_word_size(),
                        dataset.japanese_word_size()).to(device)
...
for epoch in range(epoch_num):
    ...
    for i, (english_sentence, japanese_sentence) in enumerate(
        ...
        english_sentence = torch.tensor(english_sentence,dtype=torch.long).unsqueeze(-1).to(device)
        japanese_sentence = torch.tensor(japanese_sentence,dtype=torch.long).unsqueeze(-1).to(device)
    ...
    # 1 epoch 毎にファイルに書き出す
    model_file = "trained_model/translator_full" + str(epoch + 1) + ".model"
